...
import { ParseFailure, Parser, ParseResult, ParseResultSuccess, ParseSuccess, resultTracker, Rule } from "./Parser";
...
export function regexp<Ctx>(reg: RegExp): Parser<string, Ctx> {
  if (reg.source[0] !== "^") {
    throw new AstError.UnexpectedError(`Regular expression patterns for a tokenizer should start with '^': ${reg.source}`);
  }
  if (!reg.global) {
    throw new AstError.UnexpectedError(`Regular expression patterns for a tokenizer should be global: ${reg.source}`);
  }
  return {
...
export function rule<T, Ctx>(name: string): Rule<T, Ctx> {
  ...
  return {
    setParser(p) {
      if (parser !== null) {
        throw new AstError.UnexpectedError(`Parser already set !`);
      }
      parser = p;
    },
    parse(_parentPath, input, skip, ctx) {
      if (parser === null) {
        throw new AstError.ParserNotImplemented(name);
      }
